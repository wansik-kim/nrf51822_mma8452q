 ...
 */
static bool twi_master_issue_startcondition(void)																//SCL == high   ->					SDA (high -> low)
{
#if 0
    if (TWI_SCL_READ() == 1 && TWI_SDA_READ() == 1)													//SCL - high   ->					 	SDA - high
    {
        // Pull SDA low
        TWI_SDA_LOW();																																					//SCL == high   ->					SDA - low
    }
 ...
 * @retval true Clocking succeeded
 */
static bool twi_master_issue_stopcondition(void)																//SCL == high   ->					SDA (low -> high)
{
#if 0
    if (TWI_SCL_READ() == 1 && TWI_SDA_READ() == 1)													//SCL - high && SDA - high
    {
        ...
        TWI_SDA_HIGH();																																					//SCL - high && SDA - high
    }
    else if (TWI_SCL_READ() == 1 && TWI_SDA_READ() == 0)									//SCL - high && SDA - low
    {
        // Pull SDA high while SCL is high to issue STOP
        TWI_SDA_HIGH();																																					//SCL - high && SDA - high
    }
    else if (TWI_SCL_READ() == 0 && TWI_SDA_READ() == 0)									//SCL - low && SDA - low
    {
        if (!twi_master_wait_while_scl_low())
        {
            return false;
    ...
    else if (TWI_SCL_READ() == 0 && TWI_SDA_READ() == 1)									//SCL - low && SDA - high
    {
        TWI_SDA_LOW();
        TWI_DELAY();
